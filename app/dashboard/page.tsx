...
const topic3: Topic = {
  ...
  part2: [
    ...
    { id:'q5', type:'boolean', statement:'Ở người, bệnh máu khó đông: người phụ nữ bình thường nhưng ông ngoại bị bệnh, khả năng người em trai của bà bị bệnh là 50%.', correct:true, explanation:'✓ ��úng! Ông ngoại XᵃY → mẹ XᴬXᵃ (dị hợp) → xác suất con trai bị bệnh = 50% (vì mẹ truyền Xᵃ với xác suất 1/2).' },
...
export default function GiaSuAI() {
  ...
  const saveQuizAttempt = async () => {
    if (!currentTopic || mode !== 'results') return;
    
    try {
      const totalQuestions = score.p2Total;
      const correctAnswers = score.part2;
